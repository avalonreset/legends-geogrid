...
- **Legible basemap attribution:** Provider attribution for raster basemaps (such as OpenStreetMap or Google) must remain unoccluded, legible, and legally compliant—either by preserving the bottom credit strip clear of marker collisions (`preserve-bottom-strip`), or via explicit caption and metadata attribution. For schematic coordinate maps without a street basemap, label as 'Schematic coordinate map — no street basemap' directly in caption and metadata.
